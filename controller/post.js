...
PostController.prototype.delete = function(id, callback){
	...
	Post.findOne({'_id': new mongoose.Types.ObjectId(id)}).remove(function(err,post){
		if(err) return callback(err);
...
PostController.prototype.selectById = function(id, callback){
	Post.findOne({'_id': new mongoose.Types.ObjectId(id)}).populate('comments').exec(function(err,post){
		if (err) return callback(err);
		console.log('From post controller: '+post);
		if (!post) return callback(null, {'success': false, 'message': 'Post nao encontrado'});
